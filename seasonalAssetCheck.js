...
class seasonalAssetCheck {
  static showCheckSeasonlMenu(parameter) {
    ...
    messageHandler.postMessageOnlyVisible(channelID, "", blockGenerator.seasonalAssetsCheckBlock(), userID);
  }
  ...
  static checkSeasonalList(payload) {
    ...
    const env = environment == "dev" ? S3_DEV
      : (environment == "qa" ? S3_QA
        : (environment == "prod" ? S3_PROD : ""));
    if (env == "") {
      return;
    }
    ...
    SEASON_LIST.forEach((season) => {
      try {
        ...
        switch (feature) {
          case "DesktopTheme":
            assetsLink = this.getDesktopThemeAssetLink(env, versionString, season);
            break;
          case "CheckIn":
            assetsLink = this.getCheckInAssetLink(env, versionString, season);
            break;
          case "MatchMakerWidget":
            assetsLink = this.getMatchMakerWidgetAssetLink(env, versionString, season);
            break;
          case "MatchMaker":
            assetsLink = this.getMatchMakerAssetLink(env, versionString, season);
            break;
          case "Shop":
    ...
    messageHandler.postMessageWithoutBlock(payload.container.channel_id, environment + " s3에 업로드 되어있는 " + feature + ":\n" + text);
  }

  static getCheckInAssetLink(env, versionString, season) {
    return env + "seasonal_assets/" + versionString + "/CheckIn/" + season + "/chunks/mobile/nomin/V1/popup/seasonalevent/checkin/CheckIn.json"
  }

  static getDesktopThemeAssetLink(env, versionString, season) {
    return env + "seasonal_assets/" + versionString + "/DesktopTheme/" + season + "/chunks/desktop/nomin/V1/popup/seasonalevent/desktoptheme/DesktopTheme.json"
  }

  static getMatchMakerAssetLink(env, versionString, season) {
    return env + "seasonal_assets/" + versionString + "/MatchMaker/" + season + "/chunks/mobile/nomin/V1/popup/seasonalevent/matchmaker/MatchMaker.json"
  }

  static getMatchMakerWidgetAssetLink(env, versionString, season) {
    return env + "seasonal_assets/" + versionString + "/MatchMaker/" + season + "/widget/Widget_Seasonal_MatchMaker.json"
  }

  static getShopAssetLink(env, versionString, season) {
    return env + "seasonal_assets/" + versionString + "/Shop/" + season + "/BuyChips_seasonal.json"
  }

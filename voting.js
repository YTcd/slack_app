class voting {
  ...
  static updateVotesReceived(payload) {
    ...
    const url =
      "https://slack.com/api/users.profile.get?user=" + userID + "&pretty=1";
    ...
    for (let i = 0; i * 2 + 2 < block.length; i++) {
      ...
      if (
        block &&
        block[subjectIndex] &&
        block[subjectIndex].text &&
        block[subjectIndex].text.text &&
        block[subjectIndex].text.text.indexOf(" " + userName) > -1
      ) {
        currentIndex = subjectIndex;
        block[subjectIndex].text.text = block[subjectIndex].text.text.replace(
          " " + userName,
          ""
        );
        const chosenCnt =
          block[subjectIndex + 1].elements[0].text.split("표")[0];
    ...
    if (currentIndex != itemIndex * 2 + 2) {
      ...
      const chosenCnt =
        block[itemIndex * 2 + 3].elements[0].text.split("표")[0];
  ...
  static addVoteItem(payload) {
    ...
    if (inputValue != "") {
      ...
      block.push(blockGenerator.generateBlock1(newItemIndex, inputValue + ""));
      block.push(blockGenerator.generateBlock2());
